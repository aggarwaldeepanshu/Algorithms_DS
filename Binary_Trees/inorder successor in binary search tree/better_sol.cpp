...
void inorder_successor_recur(struct node *,struct node **);
...
inorder_successor_recur(root,&ptr);
}

void tree::inorder_successor_recur(struct node *root,struct node **ptr)
{
if(root)
{
inorder_successor_recur(root->rchild,ptr);

root->next=*ptr;
*ptr=root;

inorder_successor_recur(root->lchild,ptr);
}
}
